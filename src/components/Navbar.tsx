...
export const Navbar: React.FC = () => {
  ...
  return (
    <header className="sticky top-0 z-40 w-full glass-nav bg-white/85 backdrop-blur-md border-b border-slate-200 shadow-sm">
        ...
        <div className="flex items-center justify-between h-16 gap-3">
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <span className="hidden xl:inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-mono bg-blue-50 text-blue-700 border border-blue-200 font-semibold tracking-tight">
              v2.0 • Sovereign Public Rail
            </span>
          </div>

          {/* Center: View Switcher Nav Pills (When in App Section) */}
          {appSection === "app" ? (
            <nav className="hidden md:flex items-center p-1 rounded-2xl bg-slate-100/90 border border-slate-200/80 shadow-inner">
              {navTabs.map((tab) => {
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-xl text-xs font-semibold bg-slate-50 border border-slate-200 text-slate-700 hover:border-blue-400 hover:bg-white transition-colors shadow-sm"
              >
                <MapPin className="w-3.5 h-3.5 text-blue-600 shrink-0" />
                <span className="max-w-[90px] sm:max-w-[120px] truncate">{selectedCity.split(",")[0]}</span>
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-xs font-semibold bg-slate-50 border border-slate-200 text-slate-700 hover:border-blue-400 transition-colors shadow-sm"
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-xl text-xs font-bold bg-slate-100 hover:bg-slate-200 border border-slate-200 text-slate-800 transition-colors"
              >
                <div className="w-5 h-5 rounded-full bg-blue-600 text-white flex items-center justify-center text-[10px] font-bold">
                  {currentUser?.role === "artisan" ? "🛠️" : "👤"}
                </div>
                <span className="hidden sm:inline capitalize">
                  {currentUser ? (currentUser.role === "artisan" ? "Artisan" : "Citizen") : "Login"}
                </span>
                <ChevronDown className="w-3 h-3 text-slate-400" />
              </button>

              {userDropdownOpen && (
                <div className="absolute right-0 mt-2 w-56 py-2 bg-white border border-slate-200 rounded-2xl shadow-2xl z-50 animate-in fade-in zoom-in-95 text-xs">
                  <div className="px-3 py-2 border-b border-slate-100 mb-1">
                    <span className="text-[10px] font-bold text-slate-400 block uppercase">
                      Current Profile
                    </span>
                    <strong className="text-slate-900 text-xs block truncate font-bold">
                      {currentUser ? currentUser.name : "Guest Citizen"}
                    </strong>
                    <span className="text-[10px] text-blue-600 font-medium capitalize">
                      {currentUser?.role || "Visitor"} Account
                    </span>
                  </div>
                  ...
                  <div className="px-2 space-y-1">
                    <button
                      ...
                      className="w-full text-left px-2.5 py-1.5 rounded-xl hover:bg-slate-100 text-slate-700 flex items-center justify-between"
                    >
            ...
            {appSection === "landing" ? (
              <button
                ...
                className="flex items-center gap-1.5 px-3.5 sm:px-4 py-1.5 rounded-xl text-xs font-bold btn-glossy-blue shadow-sm active:scale-95 transition-all duration-150"
              >
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3.5 sm:px-4 py-1.5 rounded-xl text-xs font-bold btn-glossy-green shadow-sm active:scale-95 transition-all duration-150"
              >
                <Sparkles className="w-3.5 h-3.5" />
